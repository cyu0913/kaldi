...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        "Accumulate stats for training a diagonal-covariance GMM, with state level posterior as input.\n"
        "Usage:  gmm-global-acc-stats [options] <model-in> <feature-rspecifier> <posterior-rspecificer>"
        "<stats-out>\n"
        "e.g.: gmm-global-acc-stats 1.mdl scp:train.scp arK:post.ark 1.acc\n";
    ...
    for (; !feature_reader.Done(); feature_reader.Next()) {
      ...
      int32_t file_frames = mat.NumRows();
      ...
      if (gselect_rspecifier != "") {
        ...
        const std::vector<std::vector<int32_t> > &gselect =
        ...
        for (int32_t i = 0; i < file_frames; i++) {
          BaseFloat weight = (weights.Dim() != 0) ? weights(i) : 1.0;
          if (weight == 0.0) continue;
          file_weight += weight;
          SubVector<BaseFloat> data(mat, i);
          const std::vector<int32_t> &this_gselect = gselect[i];
          int32_t gselect_size = this_gselect.size();
          ...
          for (int32_t j = 0; j < loglikes.Dim(); j++)
            gmm_accs.AccumulateForComponent(data, this_gselect[j], loglikes(j));
        }
      } else { // no gselect..
        for (int32_t i = 0; i < file_frames; i++) {

          // from state aligned posterior to global gmm posterior prob  
          Vector<BaseFloat> post_vec(gmm.weights().Dim());
          post_vec.SetZero();
          int32_t pdf_id = posterior[i][0].first;
          post_vec(pdf_id)=1.0;   
          ...
          file_weight += weight;
          file_like += weight * 1;
            gmm_accs.AccumulateFromPosteriors(mat.Row(i), post_vec);
