...
class ForwardBackwardFstImpl :  public FstImpl<A> {
 ...
 public:
  ...
  void DeleteStates(const vector<StateId>& dstates) {
    ...
    for (size_t arc = 0; arc < arcs_to_delete.size();++arc) {
      delete arcs_to_delete[arc];
    }
    // Fix start state
    if (Start() != kNoStateId){
  ...
  void DeleteArcs(StateId s, size_t n) {
    ...
    for (size_t i;i < other_states.size(); i++) {
      states_[other_states[i]]->DeleteArcs<!input_arcs>(arcs_to_delete);
    }
    for (size_t arc = 0; arc < arcs_to_delete.size();++arc) {
      delete arcs_to_delete[arc];
    }
...
class ForwardBackwardFst :
 ...
 public:
  ...
  size_t NumInputEpsilons(StateId s) const {
    ...
    return GetImpl()->NumInputEpsilons(s);
  }
  ...
  size_t NumOutputEpsilons(StateId s) const {
    ...
    return GetImpl()->NumOutputEpsilons(s);
  }
