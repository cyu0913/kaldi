// fgmmbin/fgmm-global-gselect-to-post-mat.cc
...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        ...
        "\n"
        "Usage:  fgmm-global-gselect-to-post-mat [options] <model-in> <feature-rspecifier> "
        "<gselect-rspecifier> <post-wspecifier>\n"
        "e.g.: fgmm-global-gselect-to-post-mat 1.ubm ark:- 'ark:gunzip -c 1.gselect|' ark:- postmat_outdir arK:- \n";
    ...
    po.Register("binary", &binary, "Write posterior matrix in binary mode, defalt is false");
    ...
    // write out as matrix    
    std::string postmat_wxdir = po.GetArg(5);

    // write out as ark format (like mfcc feats)
    std::string wspecifier = po.GetArg(6);
    ...
    for (; !feature_reader.Done(); feature_reader.Next()) {
      ...
      Matrix<float> post_mat(num_frames,fgmm.NumGauss());
      ...
      for (int32 t = 0; t < num_frames; t++) {
        ...
        if (fabs(loglikes.Sum() - 1.0) > 0.01) {
          ...
          for (int32 i = 0; i < loglikes.Dim(); i++) {
            if (loglikes(i) != 0.0) {
              ...
              post_mat(t, this_gselect[i]) = loglikes(i);
              tot_posts++;
            }
          }
        ...
        // write posterior matrix for each input file
        std::string postmat_wxfilename = postmat_wxdir + "/" + utt;
        {
        Output ko(postmat_wxfilename,binary);
        post_mat.Write(ko.Stream(),binary);
        }

        kaldi_writer.Write(utt, post_mat);
